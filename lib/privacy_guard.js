...
const CREDENTIAL_VALUE_PATTERN = /\b(?:otp|one[-\s]?time(?:\s+password)?|passcode|password|pin)\s*(?:(?:is|code|number|no\.?)\s*|[:=\-]\s*)(?:\d{4,8}|[A-Za-z0-9!@#$%^&*]{6,})\b|\b(?:otp|one[-\s]?time(?:\s+password)?|passcode|password|pin)\s+\d{4,8}\b/gi;
