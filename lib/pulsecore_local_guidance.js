...
export function getPulseCoreLocalGuidance(input) {
  ...
  // Handle a specific, high-risk vishing pattern before the broader credential
  // guidance. This is intentionally triggered only when an RBI/bank identity is
  // combined with a request for sensitive financial information or a balance.
  const claimsRbiOrBankRole = /\b(rbi|reserve\s*bank|bank\s*(?:official|employee|manager|support)|customer\s*(?:care|support))\b/i.test(value);
  const asksForSensitiveBankInformation = /\b(bank\s*details?|account\s*details?|current\s*bank\s*balance|bank\s*balance|current\s*balance|otp|pin|cvv|password|card\s*(?:details?|number)|kyc|aadhaar|pan|screen\s*share|remote\s*access)\b/i.test(value);
  if (claimsRbiOrBankRole && asksForSensitiveBankInformation) {
    return withDisclaimer(responseFor(locale, {
      en: '**Please do not share your bank details on that call.** Based on what you described, it has strong signs of an RBI/bank impersonation phone scam (vishing): an unsolicited caller is asking for your account information or current balance in the name of checking a problem. That is not something to trust through the caller’s number.\n\n1. End the call. Do not confirm your account number, balance, card details, OTP, PIN, CVV, password, UPI PIN, Aadhaar/PAN, or install/share-screen through any app.\n2. Open your bank’s official app yourself, or call the number printed on your card or shown on the bank’s official website. Ask whether there is any actual unauthorised transaction or account issue. Do not call back the number that contacted you.\n3. If you already shared any secret, installed a remote-access app, or money moved, contact your bank immediately and report the suspected financial fraud through 1930.\n\nThe caller may sound convincing, but pressure to “check your balance” or “help recover money” is a warning sign—not proof that they are genuine.',
      hinglish: '**Bhai, is call par apni bank details bilkul share mat karo.** Tumne jo bataya, usmein RBI/bank impersonation phone scam (vishing) ke strong signs hain: caller khud call karke “paise gaye hain, balance check karna hai” bolkar account information maang raha hai. Caller ke number par is baat ko genuine maan kar trust mat karo.\n\n1. Call cut kar do. Account number, current balance, card details, OTP, PIN, CVV, password, UPI PIN, Aadhaar/PAN share mat karo. Koi app install ya screen-share bhi mat karo.\n2. Khud bank ki official app kholo, ya card ke peeche/official website par jo number hai us par call karo. Unse pucho ki sach mein koi unauthorised transaction ya account issue hai ya nahi. Caller ne jo number diya ho, us par call back mat karo.\n3. Agar tumne koi secret share kiya, remote-access app install ki, ya paise move ho gaye, to bank ko turant official channel se contact karo aur suspected financial fraud ko jaldi 1930 par report karo.\n\nCaller bahut convincing bol sakta hai, lekin “balance check” ya “paise wapas dilane” ke naam par details maangna warning sign hai—genuine hone ka proof nahi.',
      hi: '**इस call पर अपनी bank details बिल्कुल साझा न करें।** आपके बताए हुए तरीके में RBI/bank impersonation phone scam (vishing) के मजबूत संकेत हैं: caller खुद call करके “पैसे गए हैं, balance check करना है” कहकर account information माँग रहा है। Caller के number के आधार पर इसे असली न मानें।\n\n1. Call काट दें। Account number, current balance, card details, OTP, PIN, CVV, password, UPI PIN, Aadhaar/PAN साझा न करें। कोई app install या screen-share भी न करें।\n2. स्वयं bank का official app खोलें, या card के पीछे/official website पर दिए number पर call करें। उनसे पूछें कि वास्तव में कोई unauthorised transaction या account issue है या नहीं। Caller के दिए number पर call back न करें।\n3. यदि आपने कोई secret साझा किया, remote-access app install किया, या पैसे चले गए हैं, तो तुरंत bank के official channel से संपर्क करें और suspected financial fraud की सूचना जल्दी 1930 पर दें।\n\nCaller बहुत भरोसेमंद लग सकता है, लेकिन “balance check” या “पैसे वापस दिलाने” के नाम पर details माँगना warning sign है—genuine होने का प्रमाण नहीं।'
    }));
  }
