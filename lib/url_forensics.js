...
const BRAND_REGISTRY = [
  ...
  'indiapost', 'fastag', 'irctc', 'rbi', 'uidai', 'incometax'
];

const SHORTENER_HOSTS = new Set(['bit.ly', 'tinyurl.com', 't.co', 'is.gd', 'cutt.ly', 'rb.gy', 'shorturl.at']);
const ACTION_PATTERN = /\b(?:click|tap|open|verify|re-?kyc|update|install|download|pay|approve|share|submit|call|whatsapp|telegram|reply)\b|क्लिक|भुगतान|भेजें|तुरंत|చెల్లించ|క్లిక్/i;
  ...
  for (const candidate of candidates) {
    ...
    for (const brand of claims) {
      if (!looksOfficialForClaim(url.hostname, brand)) {
        addSignal(signals, `brand_host_mismatch_${brand}`, actionRequested || sensitiveRequest ? 30 : 15, `The message claims '${brand}' but the link hostname is not in its official-domain registry.`);
      }
    }
