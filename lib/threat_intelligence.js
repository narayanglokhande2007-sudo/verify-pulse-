...
const snapshotCache = { snapshot: null, index: null, expiresAt: 0 };
...
function buildIndicatorIndex(snapshot) {
  const urls = new Map();
  const domains = new Map();
  for (const indicator of snapshot.indicators) {
    if (indicator?.indicatorType === 'url') urls.set(String(indicator.indicator), indicator);
    if (indicator?.indicatorType === 'domain') domains.set(String(indicator.hostname || indicator.indicator).toLowerCase(), indicator);
  }
  return { urls, domains };
}

async function getSnapshot() {
  if (snapshotCache.snapshot && snapshotCache.index && Date.now() < snapshotCache.expiresAt) return snapshotCache;
  try {
    ...
    snapshotCache.index = buildIndicatorIndex(payload);
...
export async function lookupThreatIntelligence(text) {
  ...
  if (!cachedSnapshot) return { matched: false, checked: false, matches: [] };

  const matches = [];
  const seenIndicators = new Set();
  const addMatch = (candidate, indicator) => {
    if (!indicator || !indicatorMatches(candidate, indicator)) return;
    const key = `${indicator.indicatorType}:${indicator.indicator}`;
    if (seenIndicators.has(key)) return;
    ...
    matches.push({
      ...
      qualityTier: String(indicator.qualityTier || 'unclassified'),
  ...
  for (const candidate of candidates) {
    const canonicalUrl = `${candidate.protocol}//${candidate.hostname}${candidate.port ? `:${candidate.port}` : ''}${candidate.pathname || '/'}`;
    addMatch(candidate, cachedSnapshot.index.urls.get(canonicalUrl));
    const labels = candidate.hostname.split('.');
    for (let start = 0; start < labels.length - 1; start += 1) {
      addMatch(candidate, cachedSnapshot.index.domains.get(labels.slice(start).join('.')));
    }
  }
  ...
  return {
    ...
    checked: true,
    generatedAt: cachedSnapshot.snapshot.generatedAt,
